  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/journals/${journalId}`, {
        method: "POST", // Laravel will detect `_method` below
        body: (() => {
    ...
    <Modal show={show} onClose={onClose}>
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <TextInput label="Judul Jurnal" name="title" value={form.title} onChange={handleChange} isRequired />
          <TextInput label="Link" name="link" value={form.link} onChange={handleChange} placeholder="https://..." isRequired />
          <TextInput label="Acceptance Rate" name="acceptance_rate" type="number" value={form.acceptance_rate} onChange={handleChange} />
          <TextInput label="Decision Days" name="decision_days" type="number" value={form.decision_days} onChange={handleChange} />
          <TextInput label="Impact Factor" name="impact_factor" type="number" step="0.01" value={form.impact_factor} onChange={handleChange} />
          ...
          <div className="col-span-full">
            <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">Sampul (Cover)</label>
            {preview ? (
              <div className="relative w-[100px] h-[100px] max-w-xs">
                <img src={preview} alt="Preview" className="rounded shadow" />
                <button type="button" onClick={handleRemoveImage} className="absolute top-1 right-1 px-2 py-1 bg-red-500 text-white text-xs rounded-full">
                  ×
                </button>
              </div>
            ) : (
